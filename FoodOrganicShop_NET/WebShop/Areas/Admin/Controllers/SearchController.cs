...
namespace WebShop.Areas.Admin.Controllerst
{
	...
	public class SearchController : Controller
	{
		...
		public IActionResult FindProduct(string keyword)
		{
			...
			{
				ls = _context.Products.AsNoTracking().Include(a => a.Cat).OrderByDescending(x => x.ProductName).Take(10).ToList();
				return PartialView("ListProductsSearchPartial", ls);
			}
			else
			{
				ls = _context.Products.AsNoTracking().Include(a => a.Cat).Where(x => x.ProductName.Contains(keyword))
									  .OrderByDescending(x => x.ProductName).Take(10).ToList();
				if (ls == null) return PartialView("ListProductsSearchPartial", null);
				else return PartialView("ListProductsSearchPartial", ls);
			}
		}
