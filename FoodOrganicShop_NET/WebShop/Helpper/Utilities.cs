...
namespace WebShop.Helpper
{
    public static class Utilities
    {
        public static string StripHTML(string input)
        {
            try
            {
                if(!string.IsNullOrEmpty(input)) return Regex.Replace(input, "<.*?>", String.Empty);
            }
            catch
            {
                return null;
        ...
        public static async Task<string> UploadFile(Microsoft.AspNetCore.Http.IFormFile file, string sDirectory, string newname = null)
        {
            try
            {
                ...
                if (!supportedTypes.Contains(fileExt.ToLower()))  return null;
                else
                {
                    using (var stream = new FileStream(pathFile, FileMode.Create)) await file.CopyToAsync(stream);
                    return newname;
                }
            }
            catch
